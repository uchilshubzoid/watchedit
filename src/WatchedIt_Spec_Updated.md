...
| **2.16** | Stats data accuracy fixes. Currently watching titles included in all stats (summary count, hours, category breakdown, timeline chart) when any session falls in the selected period. Timeline chart (`buildTimePoints`) rewritten to bucket watching entries by each individual session day — a title with sessions on multiple days appears in every active day. Hours use `e.epRuntime × sessions-in-period` (not full `watchTime` string). WatcherScreen all-time quick stats pool expanded to watched + watching; hours for watching use `e.ep × e.epRuntime`. | ✅ Complete |
| **2.17** | Bug fixes. Watch Deets "Watching Since" no longer shows the title log date as the range end when no sessions exist. LogIt now creates an initial `watch_sessions` entry (dated today) when submitting a watching title with `epWatched > 0`, so those episodes are immediately visible in period stats and the timeline chart. | ✅ Complete |
| **3** | Google Drive actual sync (write/read appdata). Supabase. MAL OAuth import. Netflix CSV import. Review to Log queue. API keys server-side. **Play Store closed testing live ✅ (Jun 2026)** — promote to open/production once feedback is stable. | 🔲 |
...
| Version | Changes |
|---|---|
| 2.18 | **Watch Deets Watching Since end-date fix + LogIt initial session creation.** **Watch Deets end-date fix (`DetailView`):** "Watching Since" section was showing `start → entry.date` (the title log/add date) when a currently watching entry had a start date but no sessions. Added `watchingRealEnd = lastSesh?.date_display \|\| entry.lastWatchedDate \|\| null` (no fallback to `entry.date`). The range is now only shown when a real last-activity date exists; otherwise just the start date is displayed ("Watching Since Jan 15"). Dropped entries retain the existing `endDate` behaviour. **LogIt initial session fix (`LogItDetails`):** When submitting a new currently watching entry with `epWatched > 0`, `watch_sessions` was always saved as `[]`. This meant period filters and timeline chart bucketing could exclude the entry if a historic `watch_start_date` was set (because `parseActivityDate` returned the start date, which fell outside the 7/30-day window, and `sessionDateInRange` found nothing). Fix: when `isCurrent && epWatched > 0`, an initial session `{ ep_from: 1, ep_to: epWatched, date: todayISO, date_display: today }` is now created at submit time, anchoring the logged episodes to the day the user added the entry. |
| 2.17 | **Stats data accuracy fixes — currently watching included in all calculations.** **`filterByPeriod`:** watching entries now pass the period filter if any `watch_sessions` entry has a date within the window (via new `sessionDateInRange(e, start, end)` helper), even when `parseActivityDate` (MAX session date) falls outside the range. Previously only fully-watched/dropped entries were counted. **`watchHoursInPeriod(e, pStart, pEnd)`:** new helper used for all hours calculations in stats summary, category breakdown, and type header counts. For watching entries: filters `watch_sessions` to those in the period, counts `(ep_to - ep_from + 1)` episodes per session, multiplies by `e.epRuntime` (stored integer, mins/ep). For watched entries or when no period set: falls back to `entryWatchHours`. **`epsWatchedInPeriod(e, pStart, pEnd)`:** new helper for category breakdown episode counts. Watched entries return full `e.ep`; watching entries sum only sessions-in-period episodes. **`buildTimePoints` rewrite:** timeline chart now uses two inner helpers — `dayContrib(e, ds)` and `monthContrib(e, m, y)` — that bucket watching entries by each individual session's date, not the MAX session date. A title with sessions on May 15, May 20, and Jun 3 now appears in all three day buckets with the correct hours for that session only. Hours per day/month for watching entries: `(sessions_in_bucket × epRuntime) / 60`. All Time mode gathers timestamps from all session dates (not just `parseActivityDate`) to find the true earliest activity month. Non-watching entries retain single `parseActivityDate` attribution. **WatcherScreen quick stats:** `statsPool` expanded from `status === 'watched'` to `status === 'watched' || status === 'watching'`. Hours for watching entries: `e.ep × e.epRuntime` (episodes watched so far × stored per-ep runtime). Label renamed "Watched" → "Titles" to reflect the mixed pool. Avg Rating covers the same pool. |
| 2.16 | **WatchList watched-date fix + WatchTower Currently Watching order fix.** **WatchList watched-date bug:** `finishedDate` was hardcoded to today's date (the log date) in `LogItDetails.handleSubmit` for all new Watched entries — causing the WatchList card to always show the day of logging rather than the actual watch-end date chosen by the user. Fixed by computing `watchEndDisplay` from `watchEndDate` (the user-selected ISO date) using the same `toLocaleDateString('en-US', { month: 'short', day: 'numeric' })` format. `finishedDate` now correctly reflects the watch-end date in both new-entry and edit-mode flows; `watch_end_date` (ISO, the stats-attribution field) was always set correctly and is unchanged. The `date` field (entry log date, used for Watch Plan "added" display) is not affected. **WatchTower Currently Watching order:** `getActivityDate` for `watching` entries previously used the **last-by-position** session in `watch_sessions` as the sort key — if a user ever logged a backdated session after a more recent one the sort key would regress. Changed to iterate all sessions and return the **MAX session date**, making the Currently Watching horizontal scroll robust to out-of-order session entries. Sort remains `getActivityDate(b) - getActivityDate(a)` (most recently watched first); `slice(0, 6)` tile limit and +N overflow card unchanged. |
